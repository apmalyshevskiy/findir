 ...
 * тем же способом, что и в матчере, чтобы поведение совпадало. При регистрации
 * справочники ещё пусты, поэтому строки создаются с flow_info_id = null; статья
 * доразрешается повторным запуском сидера после применения шаблона справочников
 * (см. DictionaryTemplates::apply). Тенант может выбрать статью и вручную —
 * повторный запуск её не затрёт.
 *
