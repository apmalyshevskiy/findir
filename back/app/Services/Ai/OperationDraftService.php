...
class OperationDraftService
{
    ...
    private function run(string $db, string|array $userContent, ?string $model, array $history, array $extra = []): array
    {
        ...
        $usage = $result['_usage'] ?? [];
        // Файл узнаём по тому, что содержимое пришло массивом частей
        AiUsage::record($db, is_array($userContent) ? 'file' : 'operation',
            $model ?: config('services.routerai.model'), $usage);
        $drafts = [];
...
1a. ВЫБОР СЧЁТА ДЕБЕТА — сначала ищи специализированный счёт, и только потом расходы:
   ...
   • оплата от клиента в счёт долга → счёт расчётов с клиентами (А300);
