...
export default function DashboardPage() {
  ...
  const totalSelected = operations
    .filter(op => selected.has(op.id))
    .reduce((sum, op) => sum + parseFloat(op.amount), 0)

  // Таблица дебет/кредит по счетам
  const accountTotals = useMemo(() => {
    const map = {}
    operations.forEach(op => {
      // Дебет
      const inKey = op.in_bi_id
      if (!map[inKey]) map[inKey] = { code: op.in_bi_code, name: op.in_bi_name?.replace(/^[А-ЯA-Z]\d+\s/, ''), debit: 0, credit: 0 }
      map[inKey].debit += parseFloat(op.amount)
      // Кредит
      const outKey = op.out_bi_id
      if (!map[outKey]) map[outKey] = { code: op.out_bi_code, name: op.out_bi_name?.replace(/^[А-ЯA-Z]\d+\s/, ''), debit: 0, credit: 0 }
  ...
  return (
    <Layout>
      ...
      {/* Две колонки: таблица операций + сводка по счетам */}
      <div className="flex gap-4 items-start">

        {/* Таблица операций */}
        <div className="flex-1 bg-white rounded-xl border border-gray-100 shadow-sm min-w-0">
                ...
                <tbody>
                  {operations.map(op => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 text-sm text-gray-500 max-w-32 truncate">{op.note || '—'}</td>
                      <td className="px-3 py-3 text-right" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleEdit(op)}
                            className="text-gray-400 hover:text-blue-600 text-sm p-1 rounded hover:bg-blue-50">✎</button>
                          <button onClick={() => handleDelete(op.id)}
                            className="text-gray-400 hover:text-red-500 text-base p-1 rounded hover:bg-red-50">×</button>
                        </div>
                      </td>
                    </tr>
                  ))}
              ...
              <table className="w-full">
                <thead>
                  <tr className="text-xs text-gray-500 border-b border-gray-50">
                    <th className="text-left px-4 py-2">Счёт</th>
                    <th className="text-right px-3 py-2 text-green-600">Дт</th>
                    <th className="text-right px-3 py-2 text-red-500">Кт</th>
                  </tr>
                </thead>
                <tbody>
                  {accountTotals.map(acc => (
                    <tr key={acc.code} className="border-b border-gray-50 hover:bg-gray-50">
                      <td className="px-4 py-2">
                        <div className="text-xs font-mono font-medium text-gray-700">{acc.code}</div>
                        <div className="text-xs text-gray-400 truncate max-w-28">{acc.name}</div>
                      </td>
                      <td className="px-3 py-2 text-right text-xs font-medium text-green-600 whitespace-nowrap">
                ...
                <tfoot>
                  <tr className="border-t border-gray-200">
                    <td className="px-4 py-2 text-xs font-semibold text-gray-600">Итого</td>
                    <td className="px-3 py-2 text-right text-xs font-bold text-green-600 whitespace-nowrap">
